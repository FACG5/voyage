...
const getComment = reviewId => new Promise((resolve, reject) => {
  const sql = {
    text: 'SELECT person.username, comment.content, comment.id FROM comment JOIN person ON comment.person_id=person.id WHERE comment.review_id = $1 ORDER BY comment.id DESC ',
    values: [reviewId],
  };
  dbConnection.query(sql, (error, res) => {
    if (error) {
